...
(ert-deftest taco-tests-get-builddir-out-of-tree-builder ()
  (with-fake-files '("build.bfg")
    ...
    (with-fake-project `(vc . ,default-directory)
      (should (equal (taco-project-get-builddir) "/path/to/project/build/"))
      (let ((default-directory "/path/to/project/sub/"))
        (should (equal (taco-project-get-builddir) "/path/to/project/build/"))))
    (with-fake-project '(transient . "")
      (should (equal (taco-project-get-builddir) "/path/to/project/build/")))
    (with-fake-project nil
      (should (equal (taco-project-get-builddir) nil)))))
...
(ert-deftest taco-tests-get-builddir-in-tree-builder ()
  (with-fake-files '("Makefile")
    ...
    (should (equal (taco-project-get-builddir '(vc . "/path/to/project/")
                   ...
                   "/path/to/project/"))
    (with-fake-project `(vc . ,default-directory)
      (should (equal (taco-project-get-builddir) "/path/to/project/"))
      (let ((default-directory "/path/to/project/sub/"))
        (should (equal (taco-project-get-builddir) "/path/to/project/"))))
    (with-fake-project '(transient . "")
      (should (equal (taco-project-get-builddir) "/path/to/project/")))
...
(ert-deftest taco-tests-compile-command ()
  (with-fake-files '("build.bfg")
    ...
    (should (equal (taco-compile-command "build" :one-step t) "9k build/"))
    (should (equal (taco-compile-command "build" :trailing-space t)
                   "9k build/ && cd build/ && ninja "))
    (should (equal (taco-compile-command "build" :one-step t :trailing-space t)
                   "9k build/ "))
    (let ((default-directory "/path/to/elsewhere/"))
      (should (equal (taco-compile-command "build" :srcdir "/path/to/project/")
                     "9k build/ && cd build/ && ninja")))))
...
(ert-deftest taco-tests-compile ()
  (cl-letf (((symbol-function #'compile)
    ...
    (with-fake-files '("build.bfg")
      ;; `taco-compile'
      (should (equal (taco-compile) "9k build/ && cd build/ && ninja"))
      ...
      (should (equal (taco-project-compile `(vc . ,default-directory)
                     ...
                     "9k builddir/ && cd builddir/ && ninja"))
      (with-fake-project `(vc . ,default-directory)
        (should (equal (taco-project-compile)
                       "9k build/ && cd build/ && ninja"))
      ...
      (with-fake-project '(transient . "")
        ...
        (should (equal (call-interactively #'taco-project-compile)
                       "9k build/ && cd build/ && ninja ")))
      (with-fake-project nil
        (should-error (taco-project-compile))
        (should (equal (call-interactively #'taco-project-compile)
                       "9k build/ && cd build/ && ninja ")))
        )))
...
(ert-deftest taco-tests-bfg9000-rebuild ()
  (with-fake-files '("build.bfg" "build/build.ninja")
    (should (equal (taco-compile-command "build") "cd build/ && ninja"))
    (should (equal (taco-compile-command "build" :one-step t)
                   "cd build/ && ninja"))
    (should (equal (taco-get-builddir nil "build") "/path/to/project/build/"))))
...
(ert-deftest taco-tests-make ()
  (with-fake-files '("build.ninja")
    (should (equal (taco-compile-command "build")
                   "ninja"))
    (should (equal (taco-compile-command "build" :one-step t) "ninja"))
    (should (equal (taco-get-builddir nil "build") "/path/to/project/"))))

(ert-deftest taco-tests-absolute-builddir ()
  (with-fake-files '("build.bfg")
    (should (equal (taco-compile-command "/path/to/build")
                   "9k ../build/ && cd ../build/ && ninja"))
    (should (equal (taco-compile-command "/path/to/build" :one-step t)
                   "9k ../build/"))
...
(ert-deftest taco-tests-remote-builddir ()
  (let* ((builddir "/ssh:host:/path/to/build/")
    ...
    (with-fake-files '("build.bfg")
      (should (equal (taco-compile-command builddir)
                     (format "9k %1$s && cd %1$s && ninja" escaped-builddir)))
      (should (equal (taco-compile-command builddir :one-step t)
